...
const int PINO_AUDIO = 6;
const int LEDS[] = {8, 9, 10, 11};
const int BOTOES[] = {2, 3, 4, 5};
const int TONS[] = {NOTE_A5, NOTE_A4, NOTE_G4, NOTE_D4};

const int MAX_SEQUENCIA = 100;
...
void playMelody() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote += 2) {
    ...
    tone(PINO_AUDIO, melody[thisNote], noteDuration * 0.9);
    delay(noteDuration);
    noTone(PINO_AUDIO);
  }
}
...
void aguardarJogada() {
  ...
  while (!jogada_efetuada && !perdeu_o_jogo) {
    botao_pressionado = lerBotao();

    if (botao_pressionado != -1) {
      int indice = botao_pressionado;
      tone(PINO_AUDIO, TONS[indice]);
      digitalWrite(LEDS[indice], HIGH);

      while (digitalRead(BOTOES[indice]) == LOW) {
        delay(10);
      }
...
void setup() {
  Serial.begin(9600);
  Serial.println("Simon Says - Iniciando...");
...
void loop() {
  if (perdeu_o_jogo) {
    Serial.print("GAME OVER! Pontuação: ");
